...
**Status:** v1.1.2 (2026-09-13). Phases 0 to 4 are done (§16): auth, raw
...
- Every request asks for compact JSON (`prettyPrint=false`), in the one
  place that builds an HTTP request: a 150-page document is 7.44 MB
...
- **Identifier scan** (`make leaks`): a gate over every tracked
...
| The error classes are a closed vocabulary of ten, stated in §9 and in a comment on `gapi.Class` (the design's oldest model-facing claim) | Refuted 2026-09-05 while comparing this server's conventions with two siblings': the code emitted **fifteen**, four of which — `unknown`, `stale`, `unavailable`, `unsupported` — appeared in no document, and `ambiguous` carried two unrelated meanings, a target matching several things and a write whose outcome is unknown. Those two ask the caller to do opposite things. The vocabulary lived in comments in two packages, so nothing could disagree with it | `service.Classes` is the whole list with what each class asks the reader to do; `gapi.Classes` is the client's half and is asserted to be a subset; the `classes` gate scans every `Errorf` literal in `internal/` and fails on a class that is not listed, on a listed class nothing emits, and on a scan that reads too few files to be looking at anything. The write-outcome class is `ambiguous_outcome` now. |
...
| Release notes can be a file passed with `--release-notes` while `release.header` and `release.footer` keep wrapping it, and `changelog.disable` is the way to stop goreleaser inventing a commit list | Both refuted before either was relied on, 2026-09-13, against goreleaser v2.18.1's own documentation and then its source. `internal/pipe/changelog` returns as soon as it sees `ReleaseNotesFile` or `ReleaseNotesTmpl`, so a header or footer configured in `.goreleaser.yaml` is never appended to one — dead config that reads like a promise. And `changelog.disable` is checked in `Skip`, before the pipe runs at all: the documentation says it "will also ignore any changelog files passed via `--release-notes`, and will render an empty changelog". A sibling server carries `disable: true` today, which is exactly why its release page shows a footer and nothing else. Adopting the obvious combination — disable, plus a notes file — would have published an empty body on the next tag of all four servers | `scripts/relnotes` builds the **whole** body: the `CHANGELOG.md` section matching the tag, a rule, then the footer, with the `.mcpb` paragraph behind a flag for the three servers that ship one. The `changelog:` and `release.footer:` keys are deleted rather than left inert, and the file that replaced them carries the reason. A tag with no section, or an empty one, fails the release rather than publishing silence — asserted in `scripts/relnotes`, which also asserts the footer reaches the body, because that is the half goreleaser drops |
