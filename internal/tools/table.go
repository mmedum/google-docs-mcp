...
type TableOpInput struct {
	Op              string         `json:"op" jsonschema:"insert_table, set_cells, insert_rows, delete_rows, insert_columns, delete_columns, merge_cells, unmerge_cells, style_cells, style_columns, style_rows, pin_header_rows"`
	...
	ColumnNumbers   []int          `json:"column_numbers,omitempty" jsonschema:"delete_columns: the columns to delete; style_columns: the columns to size, 1-based"`
...
func registerTable(s *mcp.Server, d Deps) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Create and change tables in a Google Doc. Ops: insert_table (rows, columns, optional data grid, at a " +
			...
			"(background, vertical alignment, padding), style_columns (fixed width in points, or share the width evenly), " +
			"style_rows (least height, keep a row off a page break), pin_header_rows (repeat the first n rows on every page). Tables are named by handle (tbl1) from a read " +
